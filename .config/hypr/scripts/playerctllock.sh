...
get_metadata() {
	key=$1
	playerctl metadata --format "{{ $key }}" -p 'mpd' 2>/dev/null
}
...
--length)
	...
	;;
--status)
	status=$(playerctl status -p 'mpd' 2>/dev/null)
	...
	;;
--album)
	album=$(playerctl metadata --format "{{ xesam:album }}" -p 'mpd' 2>/dev/null)
	if [[ -n $album ]]; then
		length=${#album}
		lim=20
